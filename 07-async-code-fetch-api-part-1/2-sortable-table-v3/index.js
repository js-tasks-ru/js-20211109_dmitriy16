...
export default class SortableTable {
  ...
  constructor(headerCfg = {}, {url = '', isSortLocally = false, sorted = {id: headerCfg.find(item => item.sortable).id, order: 'asc'}} = {}) {
    ...
    return header.map(({id = '', title = '', sortable = false}) => {
      ...
      return `
        <div class="sortable-table__cell" data-id="${id}" data-sortable="${sortable}" data-order="${order}">
          <span>${title}</span>
          ${isOrder ? '<span data-element="arrow" class="sortable-table__sort-arrow"><span class="sort-arrow"></span></span>' : ''}
        </div>
      `;
    }).join('');
  }
  ...
  sortOnClient(id, order) {
    ...
    const actualSortType = sortType === 'string' ? (a, b) => a.localeCompare(b, ['ru', 'en'], {caseFirst: 'upper'}) : (a, b) => a - b;
    const dataSorting = [...this.data].sort((a, b) => actualSortType(a[id], b[id]) * sortDir);
